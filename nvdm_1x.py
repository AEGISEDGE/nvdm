...
""" Refined by AEGIS
    1.Add tensorflow 1.x support
    2.Add checkpoint to store the model parameters.
"""
...
dump_prefix='nvdm_'
dump_mat_R_name="nvdm_baseline_20sample_tanh_mat_r.bin"
np.random.seed(0)
...
flags.DEFINE_integer('n_sample', 5, 'Number of samples.')
...
class NVDM(object):
    ...
    def __init__(self, 
        ...
        with tf.variable_scope('encoder'): 
          self.enc_vec_nd = utils.mlp(self.x, [self.n_hidden], self.non_linearity)
          self.enc_vec=tf.nn.dropout(self.enc_vec_nd, keep_prob=0.8)
        ...
        with tf.variable_scope('decoder'):
          ...
          # multiple samples
            ...
            recons_loss_list = []
            doc_vec_list=tf.zeros([batch_size, n_topic])
            for i in xrange(self.n_sample):
              if i > 0: tf.get_variable_scope().reuse_variables()
              curr_eps = eps_list[i]
              doc_vec = tf.add(tf.multiply(tf.exp(self.logsigm), curr_eps), self.mean, name='vector')
              doc_vec_list=tf.add(doc_vec_list, doc_vec)
              logits = tf.nn.log_softmax(utils.linear(doc_vec, self.vocab_size, scope='projection'))
            ...
            self.doc_vec=tf.divide(doc_vec_list, 1.0*self.n_sample, name='doc_vec')
...
def train(sess, model, 
  ...
  for epoch in range(ckpt, training_epochs):
    ...
    if no_decent_flg<30:
      if epoch+1>10:
        if mini_ppx>print_ppx:
          mini_ppx=print_ppx
          no_decent_flg=0
          sav.save(sess, dump_prefix+str(FLAGS.n_sample)+'_'+str(FLAGS.non_linearity), global_step=epoch+1)
      	else:
      	  no_decent_flg+=1
    else:
      break
...
def main(argv=None):
    ...
    ckpt_n=0
    if tf.train.latest_checkpoint('./')!= None:
      former_saver=tf.train.Saver()
      former_saver.restore(sess, tf.train.latest_checkpoint('./'))
      ckpt_n=int(tf.train.latest_checkpoint('./')[-2:])
    if tf.train.latest_checkpoint('./')==None:
    ...
    train(sess, nvdm, train_url, test_url, FLAGS.batch_size, sav=saver, ckpt=ckpt_n)
